...
-- !! COLOQUE SUA URL AQUI (com /api/public/track no final) !!
local HUB_TRACK_ENDPOINT = "https://leaderboard-manager--guzinsamuel10.replit.app/api/public/track"
...
-- ===== HTTP (compatível Xeno, Synapse, KRNL, Delta, Wave) =====
local function getHttpFunc()
    -- Xeno / Delta / Wave / a maioria dos executores modernos
    if request then return request end
    -- Synapse X
    if syn and syn.request then return syn.request end
    -- KRNL
    if http_request then return http_request end
    -- Fallback usando HttpService (limitado, sem headers)
    return nil
end
...
local function createCounter()
    ...
    frame.Position               = UDim2.new(1, 20, 0, 12)
